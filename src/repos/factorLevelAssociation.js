...
class factorLevelAssociationRepo {
  ...
  @setErrorCode('591000')
  find = (id) => this.rep.oneOrNone('SELECT * FROM factor_level_association WHERE id = $1', id)

  @setErrorCode('592000')
  batchFind = (ids) => this.rep.any('SELECT * FROM factor_level_association WHERE id IN ($1:csv)', [ids]).then(data => {
    const keyedData = _.keyBy(data, 'id')
    return _.map(ids, id => keyedData[id])
  })

  @setErrorCode('593000')
  findByExperimentId = (experimentId) => this.rep.any('SELECT fla.* FROM factor f INNER JOIN factor_level fl ON f.id = fl.factor_id INNER JOIN factor_level_association fla ON fl.id = fla.associated_level_id WHERE f.experiment_id = $1', experimentId)
  ...
  @setErrorCode('598000')
  findByBusinessKey = (keys) => this.rep.oneOrNone('SELECT * FROM factor_level_association WHERE associated_level_id=$1 and nested_level_id=$2', keys)
  ...
  @setErrorCode('59A000')
  findNestedLevels = (associatedLevelId) => this.rep.any('SELECT fl.* FROM factor_level fl INNER JOIN factor_level_association fla ON fl.id = fla.nested_level_id WHERE fla.associated_level_id=$1', associatedLevelId)

  @setErrorCode('59B000')
  batchFindNestedLevels = (associatedLevelIds) => {
    return this.rep.any('SELECT fla.associated_level_id, fl.* FROM factor_level fl INNER JOIN factor_level_association fla ON fl.id = fla.nested_level_id WHERE fla.associated_level_id IN ($1:csv)', [associatedLevelIds])
      .then(data => {
  ...
  @setErrorCode('59C000')
  findAssociatedLevels = (nestedLevelId) => this.rep.any('SELECT fl.* FROM factor_level fl INNER JOIN factor_level_association fla ON fl.id = fla.associated_level_id WHERE fla.nested_level_id=$1', nestedLevelId)

  @setErrorCode('59D000')
  batchFindAssociatedLevels = (nestedLevelIds) => {
    return this.rep.any('SELECT fla.nested_level_id, fl.* FROM factor_level fl INNER JOIN factor_level_association fla ON fl.id = fla.associated_level_id WHERE fla.nested_level_id IN ($1:csv)', [nestedLevelIds])
      .then(data => {
