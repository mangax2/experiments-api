...
WITH temp_generate_json as (
	SELECT
	  ...
	  json_build_object(
		'questionCode',
		CASE WHEN fpl.multi_question_tag IS NOT NULL THEN fld.question_code ELSE fpl.question_code END,
		'objectType',
		object_type,
		'label',
		label,
  ...
  final_group AS (
	SELECT
	  factor_level_id,
	  array_agg(agg_array) AS final_arrays
	FROM
	  pre_final_group
	GROUP BY
	  factor_level_id
  ...
  SELECT
	json_object_agg(factor_level_id, final_arrays) AS level_details
  FROM
	final_group  
`
