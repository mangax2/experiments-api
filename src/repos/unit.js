module.exports = (rep, pgp) => ({
  ...
  batchFindAllByGroupIds: (groupIds, tx = rep) => tx.any('SELECT id, group_id, treatment_id, rep, entry_id FROM unit WHERE group_id IN ($1:csv)', [groupIds]),
  batchCreate: (units, context, tx = rep) => {
    const columnSet = new pgp.helpers.ColumnSet(
      ['group_id', 'treatment_id', 'rep', 'entry_id', 'created_user_id', 'created_date', 'modified_user_id', 'modified_date'],
      { table: 'unit' },
    )
  ...
  batchUpdate: (units, context, tx = rep) => tx.batch(
    units.map(
      u => tx.oneOrNone(
        'UPDATE unit SET (group_id, treatment_id, rep, entry_id, modified_user_id, modified_date) = ' +
        '($1, $2, $3, $4, $5, CURRENT_TIMESTAMP) WHERE id = $6 RETURNING *',
        [u.groupId, u.treatmentId, u.rep, u.entryId, context.userId, u.id],
      ),
    ),
  ),
  ...
  batchPartialUpdate: (units, context, tx = rep) => {
    const columnSet = new pgp.helpers.ColumnSet(
      ['?id', 'entry_id', 'modified_user_id', 'modified_date'],
      { table: 'unit' },
    )
