...
class GroupValueService {
  ...
  batchCreateGroupValues(groupValues, context, tx) {
    return this._validator.validate(groupValues, 'POST', tx).then(() => db.groupValue.batchCreate(groupValues, context, tx).then(data => AppUtil.createPostResponse(data)))
  }

  @Transactional('getGroupValuesByGroupId')
  getGroupValuesByGroupId(id, tx) {
    return this._groupService.getGroupById(id, tx).then(() => db.groupValue.findAllByGroupId(id, tx))
  }

  @Transactional('batchGetGroupValuesByGroupIds')
  batchGetGroupValuesByGroupIds(ids, tx) {
    return this._groupService.batchGetGroupsByIds(ids, tx).then(() => db.groupValue.batchFindAllByGroupIds(ids, tx))
  }
  ...
  batchUpdateGroupValues(groupValues, context, tx) {
    return this._validator.validate(groupValues, 'PUT', tx).then(() => db.groupValue.batchUpdate(groupValues, context, tx).then(data => AppUtil.createPutResponse(data)))
  }
