...
class RefDataSourceTypeService {
  ...
  getRefDataSourceTypesWithDataSources() {
    return db.refDataSourceType.all().then(data => this._refDataSourceService.getRefDataSources().then((refDataSources) => {
      const dataSourcesGroupedByTypeId = _.groupBy(refDataSources, rds => rds.ref_data_source_type_id)
