...
class TreatmentService {
  ...
  batchCreateTreatments(treatments, context, tx) {
    return this._validator.validate(treatments, 'POST', tx).then(() => db.treatment.batchCreate(treatments, context, tx).then(data => AppUtil.createPostResponse(data)))
  }

  @Transactional('getTreatmentsByExperimentId')
  getTreatmentsByExperimentId(id, tx) {
    return this._experimentService.getExperimentById(id, tx).then(() => db.treatment.findAllByExperimentId(id, tx))
  }
  ...
  batchUpdateTreatments(treatments, context, tx) {
    return this._validator.validate(treatments, 'PUT', tx).then(() => db.treatment.batchUpdate(treatments, context, tx).then(data => AppUtil.createPutResponse(data)))
  }
  ...
  deleteTreatmentsForExperimentId(id, tx) {
    return this._experimentService.getExperimentById(id, tx).then(() => db.treatment.removeByExperimentId(id, tx))
  }
}
