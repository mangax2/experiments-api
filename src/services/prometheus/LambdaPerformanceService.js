...
class LambdaPerformanceService {
  ...
  static setUpPrometheus = (prometheusClient, numberOfDays) => {
    ...
    const updateFunc = () => db.lambdaPerformance.getXDaysStats(numberOfDays).then((data) => {
      minRequestSizeGauge.set(labels, data.min_request_size || 0)
      maxRequestSizeGauge.set(labels, data.max_request_size || 0)
      avgRequestSizeGauge.set(labels, data.avg_request_size || 0)
      minResponseSizeGauge.set(labels, data.min_response_size || 0)
      maxResponseSizeGauge.set(labels, data.max_response_size || 0)
      avgResponseSizeGauge.set(labels, data.avg_response_size || 0)
      minResponseTimeGauge.set(labels, data.min_response_time || 0)
      maxResponseTimeGauge.set(labels, data.max_response_time || 0)
      avgResponseTimeGauge.set(labels, data.avg_response_time || 0)
