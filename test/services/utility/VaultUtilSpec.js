...
describe('configureDbCredentials', () => {
    ...
    it ('configureDbCredentials when env is non prod', () => {
        ...
        return VaultUtil.configureDbCredentials('nonprod' ,vaultConfig).then((p)=>{
            sinon.assert.calledWithExactly(httpUtilPostStub,'https://my.vault.services/v1/auth/approle/login',[{headerName:'Accept', headerValue:'application/json'}],JSON.stringify(body))

            sinon.assert.calledWithExactly(httpUtilGetStub,'https://my.vault.services/v1/secret/cosmos/experiments-api/np/db',[{headerName:'X-Vault-Token', headerValue:'token'}])
    ...
    it ('configureDbCredentials when env is prod', () => {
        ...
        return VaultUtil.configureDbCredentials('prod' , vaultConfig).then((p)=>{
            sinon.assert.calledWithExactly(httpUtilPostStub,'https://my.vault.services/v1/auth/approle/login',[{headerName:'Accept', headerValue:'application/json'}],JSON.stringify(body))
            sinon.assert.calledWithExactly(httpUtilGetStub,'https://my.vault.services/v1/secret/cosmos/experiments-api/prod/db',[{headerName:'X-Vault-Token', headerValue:'token'}])
